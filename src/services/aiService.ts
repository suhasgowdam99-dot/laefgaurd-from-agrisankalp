...
export const analyzeLeaf = async (base64Image: string, onRetry?: (msg: string) => void, retryCount = 0): Promise<DetectionResult> => {
  try {
    ...
    if (!contentType || !contentType.includes("application/json")) {
      // If we get HTML instead of JSON, it's a Vercel 504/404 page.
      if (retryCount < 5) {
        const msg = `Server is busy... retrying in 8s (Attempt ${retryCount + 1})`;
        if (onRetry) onRetry(msg);
        await new Promise(r => setTimeout(r, 8000));
        return analyzeLeaf(base64Image, onRetry, retryCount + 1);
      }
      throw new Error("Server returned HTML instead of JSON. Please try again in 30 seconds.");
    }
    ...
    if (response.status === 503 && retryCount < 6) {
      ...
      const msg = `AI Model is waking up... (${waitTime}s remaining)`;
      if (onRetry) onRetry(msg);
      await new Promise(r => setTimeout(r, 8000));
      return analyzeLeaf(base64Image, onRetry, retryCount + 1);
    }
    ...
    if (Array.isArray(result) && result.length > 0) {
      ...
      const mappedData = DISEASE_MAP[label] || {
        ...
        advice: "Specialized neural diagnosis complete. Results are based on visual pathogen patterns.",
        severity: "medium"
      };
    ...
    throw new Error("Neural Scan failed to return valid data.");
  } catch (err: any) {
    console.error("Neural Guard Error:", err);
    return {
      name: "Scan Interrupted",
      confidence: "0%",
      advice: `Status: ${err.message}. The AI requires a stable connection. Please try again.`,
      severity: "high",
      status: 'error'
    };
  }
